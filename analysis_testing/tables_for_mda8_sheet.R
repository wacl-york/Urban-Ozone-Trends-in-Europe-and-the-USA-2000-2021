...
#tables = dbListTables(con)[str_detect(dbListTables(con), "piecewise_stats_freeTau")]
tables = "piecewise_stats_freeTau_mda8_anom_cold"
i = 1
...
mda8_slope_uncertainty_count = mda8_all |>
  ...
  filter(direction == "decreasing") |>
  filter(uncertainty %in% c("medium_certainty", "high_certainty")) |>
  filter(slopeBin %in% c("moderate_increase", "large_increase", "very_large_increase",
                         "moderate_decrease", "large_decrease", "very_large_decrease")) |>
  filter(year == 2019) |>
  filter(map_region == "US")
