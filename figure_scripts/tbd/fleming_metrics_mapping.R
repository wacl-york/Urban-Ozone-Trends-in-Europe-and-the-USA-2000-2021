...
metrics_binned = metrics |>
  mutate(year = year(date),
         yr_range = case_when(year %in% c(2000:2004) ~ "2000-2004",
                              year %in% c(2005:2009) ~ "2005-2009",
                              year %in% c(2010:2014) ~ "2010-2014",
                              year %in% c(2015:2021) ~ "2015-2021",
                              .default = NA)) |>
  select(-c(year,x,date,name)) |>
  group_by(yr_range, metric, station_id, latitude, longitude, country, timezone) |>
...
pdf(here("analysis","metrics_inspect_new.pdf"), width = 16, height = 9)
