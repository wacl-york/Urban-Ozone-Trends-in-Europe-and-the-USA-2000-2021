...
limUS = tibble(lng = c(-130,-50), lat = c(0,50)) |>
...
for(i in 1:length(tables)){
  ...
  tableName = tables[i]
  tableName_short = gsub("piecewise_stats_freeTau_", "", tableName)
  ...
  plotList[[paste0("geu_", tableName)]] = ggplot() +
    ...
    theme(panel.background = element_rect(fill = "white"),
          ...
          legend.byrow = T)+
    ggtitle(tableName_short)


  plotList[[paste0("gus_", tableName)]] = ggplot() +
    geom_sf(data = world, fill = "white")+
    geom_sf(data = plotDat,
            mapping = aes(colour = pvStr),
            linewidth = 0.75,
  ...
  dirOut = data_path("figures", "o3_map_selected")
...
combined_plot_eu <- (plotList[[1]] | plotList[[5]] | plotList[[3]]) +
  plot_layout(guides = "collect") & theme(legend.position = "bottom")

combined_plot_us <- (plotList[[2]] | plotList[[6]] | plotList[[4]]) +
  plot_layout(guides = "collect") & theme(legend.position = "bottom")


dirOut = "/users/bsn502/scratch/TOAR_paper/figures/o3_mda8_final_figures"

fileOutEU = here::here(dirOut, "combined_mda8_all_warm_cold_eu.pdf")
fileOutUS = here::here(dirOut, "combined_mda8_all_warm_cold_us.pdf")

pdf(fileOutEU,width = 24, height = 16)
print(combined_plot_eu)
dev.off()

pdf(fileOutUS,width = 24, height = 16)
print(combined_plot_us)
dev.off()
...
combined_plot_all <- (
  (plotList[[1]] | plotList[[5]] | plotList[[3]]) /
    (plotList[[2]] | plotList[[6]] | plotList[[4]])
) +
...
fileOutALL = here::here(dirOut, "combined_mda8_all_warm_cold_all_0.5_only.pdf")

pdf(fileOutALL,width = 24, height = 16)
...
for(i in 1:length(tables)){
  ...
  tableName = tables[i]
  tableName_short = gsub("piecewise_stats_freeTau_mda8_anom_", "", tableName)
  if(str_detect(tableName, "metric")){
    groupVars = c("station_id", "name", "tau", "metric")
    facetFormula = metric~factor(year)
  }else{
