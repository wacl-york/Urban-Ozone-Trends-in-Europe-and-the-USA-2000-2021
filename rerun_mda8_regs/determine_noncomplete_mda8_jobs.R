...
get_job_status = function(array_id, format = "jobid%-30,jobname%-30,state"){
...
jobNotComplete = jobStatus |>
  mutate(
    array_offset = JobName |>
      str_remove("toar_regression_mda8_sub_") |>
      as.numeric(),
    slurm_array_task_id = word(JobID, 2, sep = "_")
  ) |>
  filter(State != "COMPLETED")

write.csv(jobNotComplete, "rerun_mda8_regs/mda8_jobNotComplete.csv", row.names = F)
...
message = c(
  "#!/usr/bin/env bash",
  "#SBATCH --job-name=toar_regression_mda8_rerun", # Job name",
  ...
  "",
  "# Commands to run",
  paste0('Rscript --vanilla /mnt/scratch/users/',user,'/TOAR_paper/rerun_mda8_regs/5_0_1_qr_scenario_jobscript_mda8_rerun.R $SLURM_ARRAY_TASK_ID ',
         ' /mnt/scratch/projects/chem-cmde-2019/toar/data/regressions_mda8_o3/')
            )


writeLines(message, "rerun_mda8_regs/rerun_mda8_regs.sbatch")
