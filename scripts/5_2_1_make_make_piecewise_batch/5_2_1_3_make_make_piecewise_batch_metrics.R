...
message = c("#!/usr/bin/env bash",
            "#SBATCH --job-name=toar_make_piecewise_metrics # Job name",
            ...
            paste0("#SBATCH --array=0-",nrow(metric_station)-1,"          # Array range"),
            ...
            "",
            "# Commands to run",
            paste0('Rscript --vanilla /mnt/scratch/users/',user,'/TOAR_paper/scripts/5_3_0_create_piecewise/5_3_0_3_create_piecewise_metrics.R $SLURM_ARRAY_TASK_ID')
)

data_file = file(paste0('/mnt/scratch/users/',user,'/TOAR_paper/sbatch/run_makepiecewise_metrics.sbatch'), open = "wt")
