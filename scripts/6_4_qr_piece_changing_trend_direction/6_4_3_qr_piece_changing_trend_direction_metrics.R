...
qr_reg =  tbl(con, "qr_regressions_metrics")|>
  filter(stat == "slope",
         type == "fit") |>
  select(-stat, -type)
...
qr_piece = min_aic |>
  anti_join(tbl(con, "remove_sites"), by = "station_id") |>
  left_join(met_data, by = "station_id") |>
  left_join(qr_reg, by = c("station_id", "scenario_idx", "metric")) |>
  ...
  group_by(station_id, scenario_idx, metric, tau) |>
  collect() |>
  arrange(startYear) |>
  mutate(n = row_number()) |>
...
dbWriteTable(con, "trend_flip_metrics", qr_piece, overwrite = T)
