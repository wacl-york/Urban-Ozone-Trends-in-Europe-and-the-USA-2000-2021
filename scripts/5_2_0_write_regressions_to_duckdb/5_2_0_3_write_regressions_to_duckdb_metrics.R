...
read_csv_write_to_db = function(filenames){
  ...
  for(i in 1:length(filenames)){
    ...
    if(!dbExistsTable(con, "qr_regressions_metrics")){
      dbWriteTable(con, "qr_regressions_metrics", temp, overwrite = TRUE)
    }else{
      dbAppendTable(con, "qr_regressions_metrics", temp)
    }
...
reg_filenames =  system('find /mnt/scratch/projects/chem-cmde-2019/toar/data/regressions_metrics -type f -name "*.csv"', intern = TRUE)
