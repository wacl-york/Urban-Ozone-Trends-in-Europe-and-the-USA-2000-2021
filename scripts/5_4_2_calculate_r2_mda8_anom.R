...
piecewise_mda8_anom = tbl(con, "piecewise_mda8_anom") |>
  filter(tau == 0.5) |>
  left_join(
...
anom = tbl(con, "mda8_o3_anom") |>
  select(date, station_id, name, mda8_anom, x)
...
anom_piecewise_mda8 = piecewise_mda8_anom |>
  left_join(anom,
  ...
  select(x, date,station_id, name, scenario_idx,piece, mda8_anom, reg, value) |>
  filter(!is.na(date))

# calculate the R^2 between the tau=0.5 QRs (normal and piecewise) and the anomaly timeseries
anom_piecewise_mda8_r2 = anom_piecewise_mda8 |>
  select(-x, -date) |>
  group_by(station_id, name, reg, scenario_idx) |>
  summarise(r2 = cor(mda8_anom, value)^2) |>
  ungroup()

# Put anom and loess in the same data.frame
anom_loess = anom |>
  left_join(
    tbl(con, "loess_mda8_anom"),
    by = c("x", "station_id", "name")
  ) |>
  select(x, date, station_id, name, mda8_anom, value = loess) |>
  mutate(reg = "loess")
...
regs = union_all(anom_piecewise_mda8, anom_loess) |>
  collect()

dbWriteTable(con, "reg_mda8_anom",regs, overwrite = T)

rm(regs)
gc()

# get r2 from all regressions in the same data.frame
regs_r2 = union_all(anom_piecewise_mda8_r2, anom_loess_r2) |>
  collect()

dbWriteTable(con, "reg_mda8_anom_r2",regs_r2, overwrite = T)
