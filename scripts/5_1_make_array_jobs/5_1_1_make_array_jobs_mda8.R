...
if(!dir.exists(here::here('reg_batch_mda8'))){
  dir.create(here::here('reg_batch_mda8'))
}
...
for(i in 1:(fullArray+1)){
  ...
  fileOut = file.path("reg_batch_mda8", paste0("toar_regression_mda8_",array_idx_offset,".sbatch"))

  fileNameList[[i]] = fileOut



  jobname = paste0("#SBATCH --job-name=toar_regression_mda8_sub_",array_idx_offset," # Job name")
  ...
  message = c(message,
              c("# Abort if any command fails",
                ...
                "",
                "# Commands to run",
                paste0('Rscript --vanilla /mnt/scratch/users/',user,'/TOAR_paper/scripts/5_0_1_qr_scenario_jobscript_mda8.R $SLURM_ARRAY_TASK_ID ',
...
message = c("#!/usr/bin/env bash",
            "#SBATCH --job-name=toar_regression_mda8_main               # Job name",
            "#SBATCH --ntasks=1                      # Number of MPI tasks to request",
            "#SBATCH --cpus-per-task=1               # Number of CPU cores per MPI task",
            ...
            "",
            "# Commands to run",
            paste0("Rscript --vanilla /users/",user,"/scratch/TOAR_paper/hpc_scripts/run_all_mda8.R")
)



data_file = file(file.path("sbatch","runall_reg_mda8.sbatch"), open = "wt")
writeLines(message, con = data_file)
close(data_file)
