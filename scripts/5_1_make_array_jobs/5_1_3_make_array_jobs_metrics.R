...
scenarios = tbl(con, "mda8_metric_scenarios") |>
  collect()
...
if(!dir.exists(here::here('reg_batch_metrics'))){
  dir.create(here::here('reg_batch_metrics'))
}
...
for(i in 1:(fullArray+1)){
  ...
  fileOut = file.path("reg_batch_metrics", paste0("toar_regression_metrics_",array_idx_offset,".sbatch"))

  fileNameList[[i]] = fileOut



  jobname = paste0("#SBATCH --job-name=toar_regression_metrics_sub_",array_idx_offset," # Job name")
  ...
  message = c("#!/usr/bin/env bash",
              ...
              "#SBATCH --output=toar_regression_metrics/%x_log/%a/%x-%j.log       # Standard output log",
              "#SBATCH --error=toar_regression_metrics/%x_err/%a/%x-%j.err        # Standard error log")
...
message = c("#!/usr/bin/env bash",
            "#SBATCH --job-name=toar_regression_metrics_main               # Job name",
            "#SBATCH --ntasks=1                      # Number of MPI tasks to request",
            "#SBATCH --cpus-per-task=1               # Number of CPU cores per MPI task",
            ...
            "",
            "# Commands to run",
            paste0("Rscript --vanilla /users/",user,"/scratch/TOAR_paper/hpc_scripts/run_all_metrics.R")
)



data_file = file(file.path("sbatch","runall_reg_metrics.sbatch"), open = "wt")
writeLines(message, con = data_file)
close(data_file)
