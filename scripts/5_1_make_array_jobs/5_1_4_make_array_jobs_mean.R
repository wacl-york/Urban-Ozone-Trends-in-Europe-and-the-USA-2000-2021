...
if(!dir.exists("reg_batch_mean")){
  dir.create("reg_batch_mean")
}
...
for(i in 1:(fullArray+1)){
  ...
  fileOut = file.path("reg_batch_mean", paste0("toar_regression_mean_",array_idx_offset,".sbatch"))

  fileNameList[[i]] = fileOut



  jobname = paste0("#SBATCH --job-name=toar_regression_mean_sub_",array_idx_offset," # Job name")
...
message = c("#!/usr/bin/env bash",
            "#SBATCH --job-name=toar_regression_mean_main               # Job name",
            "#SBATCH --ntasks=1                      # Number of MPI tasks to request",
            "#SBATCH --cpus-per-task=1               # Number of CPU cores per MPI task",
