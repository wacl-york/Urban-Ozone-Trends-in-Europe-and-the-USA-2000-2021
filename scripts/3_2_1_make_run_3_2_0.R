...
message = c("#!/usr/bin/env bash",
            ...
            "",
            "# Commands to run",
            paste0('Rscript --vanilla /mnt/scratch/users/',user,'/TOAR_paper/scripts/3_2_0_parse_best_regs_freeTau.R $SLURM_ARRAY_TASK_ID')
)

data_file = file(paste0('/mnt/scratch/users/',user,'/TOAR_paper/sbatch/run_3_2_0.sbatch'), open = "wt")
