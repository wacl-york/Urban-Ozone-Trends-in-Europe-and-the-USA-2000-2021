...
outputFile = here(data_path(), "piecewise", "logs","fixedTau","%x_%j_%a.log")
errFile = here(data_path(), "piecewise", "logs","fixedTau","%x_%j_%a.err")
...
message = c("#!/usr/bin/env bash",
            ...
            "",
            "# Commands to run",
            paste0('Rscript --vanilla /mnt/scratch/users/',user,'/TOAR_paper/scripts/3_3_0_parse_best_regs_fixedTau.R $SLURM_ARRAY_TASK_ID')
)

data_file = file(paste0('/mnt/scratch/users/',user,'/TOAR_paper/sbatch/run_3_3_0.sbatch'), open = "wt")
