...
read_csv_write_to_db = function(filenames){
  ...
  for(i in 1:length(filenames)){
    ...
    if(!dbExistsTable(con, "qr_regressions_mda8")){
      dbWriteTable(con, "qr_regressions_mda8", temp, overwrite = TRUE)
    }else{
      dbAppendTable(con, "qr_regressions_mda8", temp)
    }
...
reg_filenames =  system('find /mnt/scratch/projects/chem-cmde-2019/toar/data/regressions_mda8_o3 -type f -name "*.csv"', intern = TRUE)
