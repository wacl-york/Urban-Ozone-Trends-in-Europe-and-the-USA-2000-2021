...
piecewise = tbl(con, "piecewise_mean") |>
  filter(tau == 0.5) |>
  left_join(
...
dbWriteTable(con, "reg_anom_mean",regs, overwrite = T)
...
dbWriteTable(con, "reg_anom_r2_mean",regs_r2, overwrite = T)
