...
best_regs = tbl(con, "reg_mda8_anom_r2") |>
...
pdf(here("analysis","mda8_anom_reg_inspect_2.pdf"), width = 16, height = 9)
for(i in 1:nrow(toDo)){
  ...
  idx_pqr_1 = tbl(con, "reg_mda8_anom_r2") |>
    group_by(station_id, name, reg) |>
    filter(r2 == max(r2, na.rm = TRUE),
           name == !!nm,
           reg %in% c("pqr_1")) |>
  ...
  idx_pqr_2 = tbl(con, "reg_mda8_anom_r2") |>
    group_by(station_id, name, reg) |>
    filter(r2 == max(r2, na.rm = TRUE),
           name == !!nm,
           reg %in% c("pqr_2")) |>
