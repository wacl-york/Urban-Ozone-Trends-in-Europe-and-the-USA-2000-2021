...
data_times = data_and_met %>%
  select(Time, Parameter, Station_country, Station_type, Station_type_of_area, filename) %>%
  group_by(Parameter, Station_country, Station_type, Station_type_of_area, filename) %>%
  mutate(start_date = min(Time),
         end_data = max(Time)) %>%
  select(-Time) %>%
  distinct()

long_term_data = data_times %>%
  filter(start_date <= "2000-01-01 00:00:00") %>%
  filter(end_data >= "2021-12-31 23:00:00")

ggplot(data_and_met %>%
         filter(Station_id == "WMOPE",
                Parameter == "O3",
                Value > -10))+
