...
requestStatusPath = here("data","us","requests_no2.csv")

stationListAllRaw = readRDS(here("data","stations_no2","stationList_US.RDS"))
...
for(i in 1:nrow(toDo)){
  ...
  for(attempt in 1:maxAttempt){
    ...
    if("tbl_df" %in% class(response)){ # if the request is successful, save the data and move to next site
      ...
      write.csv(response, here("data","us","raw_no2",filename))
